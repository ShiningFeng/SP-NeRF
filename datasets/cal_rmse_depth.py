...
from plyflatten.utils import crs_proj, rasterio_crs
from rasterio.enums import Resampling
from rasterio.transform import Affine

from modules.utils import dsm_pointwise_diff
...
def rectify_gt_dsm(gt_dir, aoi_id):
    # rectify the geo information of the gt dsm for better output_visual in QGIS
...
def CalcMAE(in_dsm_path, gt_dsm_path, gt_roi_path, gt_seg_path, rdsm_path, rdsm_diff_path, mask_out_path,
            outputMask=False):
...
def cal_rmse_depth(densedepth_file, gt_dir, aoi_id):
    ...
    get_dsm_from_dense_depth(densedepth_file, zonestring, dsm_out_path, resolution, roi_txt=roi_txt,
    ...
    diff, nanNb, totalNb = CalcMAE(dsm_out_path, gt_dsm_path, gt_roi_path, gt_seg_path, rdsm_path, rdsm_diff_path,
                                   mask_out_path, outputMask=outputMask)
    mae = np.nanmean(abs(diff.ravel()))
    print(
        f'--->>>MAE: {mae:.2f}, nan pts number in DSM transformed from depth map: {nanNb} out of {totalNb} ({(nanNb * 100.0 / totalNb):.2f}%)'
    )
